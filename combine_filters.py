...
def process_filters(urls):
    seen = set()
    total = len(urls)
    print(f"🔍 بدء معالجة {total} مصدر فلتر (إعادة محاولة تلقائية)...")
    all_rules = []
...
def save_filters_by_size(rules):
    ...
    for rule in rules:
        ...
        # تخطي القواعد الضخمة (نادرة جداً)
        if rule_size > max_size_bytes:
            print(f"⚠️ قاعدة طويلة جداً ({rule_size} بايت) سيتم تخطيها: {rule[:50]}...")
            continue
        
        # إذا إضافة القاعدة ستتجاوز الحجم، نكتب الملف الحالي ونبدأ جديداً
...
if __name__ == "__main__":
    ...
    print(f"🔐 تجاهل أخطاء SSL تلقائياً، وإعادة المحاولة حتى {MAX_RETRIES} مرات.")
    try:
